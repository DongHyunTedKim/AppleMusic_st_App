...
class PlayerViewController: UIViewController {
    ...
    /* player view controller가 메모리에 로드됐을 때 호출되는 함수 */
...
extension PlayerViewController {
    ...
    func updateTintColor() {
        print("update Tint Color, but I'm not iOS 13")
    ...
    func updatePlayButton() {
        ...
        if simplePlayer.isPlaying {
            let image = UIImage(named: "pause_fill.png")
            playControlButton.setImage(image, for: .normal)
        } else {
            let image = UIImage(named: "play_fill.png")
            playControlButton.setImage(image, for: .normal)
        }
    }
}
